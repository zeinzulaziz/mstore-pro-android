...
export default class PostDetail extends PureComponent {
  ...
  render() {
    ...
    // Safety check - if post is undefined or null, return empty view
    if (!post) {
    ...
    // Safe access to post properties with proper null checking
    const postTitle =
      post && post.title && typeof post.title.rendered !== 'undefined'
        ? post.title.rendered
        : '';
    const postContent =
      post && post.content && typeof post.content.rendered !== 'undefined'
        ? post.content.rendered
        : '';
    const videoUrl =
      post && post.content && post.content.rendered != null
      ...
      return (
        <View key={'content'} style={styles.scrollViewContent}>
          ...
          <Text style={styles.author}>
            <TimeAgo time={post.date || new Date().toISOString()} hideAgo /> ago
          </Text>
