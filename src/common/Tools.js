...
export default class Tools {
  ...
  static getImage(data, imageSize) {
    ...
    let imageURL = Constants.PlaceHolderURL;

    // 1) Try _embedded featured media first (from _embed parameter)
    if (data._embedded && data._embedded['wp:featuredmedia'] && data._embedded['wp:featuredmedia'][0]) {
      ...
      if (media.media_details) {
        imageURL = getImageSize(media.media_details);
      }
      // Fallback to source_url if available
      if (imageURL === '' && media.source_url) {
        imageURL = media.source_url;
      }
    }

    // 2) Try better_featured_image (from custom plugin)
    if (imageURL === Constants.PlaceHolderURL &&
    ...
    if (imageURL === '' || imageURL === Constants.PlaceHolderURL) {
      return Constants.PlaceHolderURL;
    }
