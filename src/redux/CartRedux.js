...
const initialState = {
  cartItems: [],
  total: 0, // This will be calculated from cartItems quantities
  totalPrice: 0,
  myOrders: [],
  isFetching: false,
};
...
export const reducer = (state = initialState, action) => {
  ...
  switch (type) {
    case types.ADD_CART_ITEM: {
      ...
      const newCartItems = isExisted
        ? state.cartItems.map(item => cartItem(item, action))
        : [...state.cartItems, cartItem(undefined, action)];
      ...
      return Object.assign(
        ...
        {
          total: calculateTotalQuantity(newCartItems),
    ...
    case types.REMOVE_CART_ITEM: {
      ...
      if (index == -1) {
        return state; // This should not happen, but catch anyway
      }
      
      const newCartItems = state.cartItems[index].quantity == 1
        ? state.cartItems.filter(cartItem => !compareCartItem(cartItem, action))
        : state.cartItems.map(item => cartItem(item, action));
...
// Helper function to calculate total quantity from cart items
